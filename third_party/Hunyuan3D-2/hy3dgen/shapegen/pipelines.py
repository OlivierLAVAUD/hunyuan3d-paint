...
class Hunyuan3DDiTPipeline:
    ...
    def from_single_file(
        ...
        **kwargs,
        ...
        # Map the checkpoint straight to CUDA when available: on low-RAM
        # machines (e.g. 8 GB) holding the ~3.8 GB fp16 checkpoint in host
        # memory on top of the model itself trips the OOM killer.
        load_device = 'cuda' if torch.cuda.is_available() else 'cpu'
        if use_safetensors:
            # parse safetensors
            import safetensors.torch
            safetensors_ckpt = safetensors.torch.load_file(ckpt_path, device=load_device)
            ckpt = {}
            ...
            ckpt = torch.load(ckpt_path, map_location=load_device, weights_only=True)
        # Build every module directly in the target dtype. Instantiating in
        # default fp32 would transiently allocate ~2x the fp16 checkpoint size
        # in RAM (and DINOv2-giant alone is ~4.4 GB in fp32).
        ...
        # Free the checkpoint tensors before the pipeline moves all weights to
        # the device, otherwise weights + checkpoint exceed the GPU's memory.
        del ckpt
        if use_safetensors:
            del safetensors_ckpt
