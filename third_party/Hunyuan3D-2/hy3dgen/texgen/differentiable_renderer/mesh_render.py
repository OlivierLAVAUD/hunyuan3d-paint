...
#
# Change: set_default_render_resolution() recomputes bake_unreliable_kernel_size,
# so lowering the render resolution also rescales the unreliable-texel kernel
# instead of keeping one sized for the original resolution.
# ---------------------------------------------------------------------------
...
class MeshRender():
    def __init__(
        ...
        # fork (Hunyuan3D-Paint): bake_unreliable_kernel_size is set by
        # set_default_render_resolution above, which now owns that derivation
        # for every resolution change (see the comment there).
    ...
    def set_default_render_resolution(self, default_resolution):
        ...
        # fork (Hunyuan3D-Paint): the unreliable-texel kernel is derived from
        # the render resolution (upstream computes it once, in __init__), so a
        # later resolution change - the service lowers this together with the
        # bake size to fit a small card - would otherwise erode the boundary
        # masks with a kernel sized for the old resolution. Recomputing here
        # keeps the eroding proportional to the image whatever the size.
